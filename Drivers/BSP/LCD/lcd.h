...
#define LCD_WR_GPIO_CLK_ENABLE()        do{ __HAL_RCC_GPIOD_CLK_ENABLE(); }while(0)   /* WR->FSMC_NWE 所在IO口时钟使能 */

#define LCD_RD_GPIO_PORT                GPIOD
#define LCD_RD_GPIO_PIN                 GPIO_PIN_4
#define LCD_RD_GPIO_CLK_ENABLE()        do{ __HAL_RCC_GPIOD_CLK_ENABLE(); }while(0)   /* RD->FSMC_NOE 所在IO口时钟使能 */
...
#define LCD_CS_GPIO_CLK_ENABLE()        do{ __HAL_RCC_GPIOG_CLK_ENABLE(); }while(0)   /* CS->FSMC_NE4 所在IO口时钟使能 */

#define LCD_RS_GPIO_PORT                GPIOG
#define LCD_RS_GPIO_PIN                 GPIO_PIN_0
#define LCD_RS_GPIO_CLK_ENABLE()        do{ __HAL_RCC_GPIOG_CLK_ENABLE(); }while(0)   /* RS->FSMC_A10 所在IO口时钟使能 */
...
/* LCD重要参数集
 * 这个结构体虽然占用了十几个字节的内存，但是却可以让我们的驱动函数 支持不同尺寸的 LCD，
 * 同时可以实现 LCD 横竖屏切换等重要功能，所以还是利大于弊的。
 * 最后声明_lcd_dev 结构体类型变量 lcddev，lcddev 在 lcd.c 中定义。
 * */
typedef struct
{
...
/*
 * TFTLCD 的 RS 接在 FSMC 的 A10 上面，CS 接在 FSMC_NE4 上，并且是 16 位数据总线。即我们使用的是 FSMC 存储器 1 的第 4 区，我们定义如下 LCD 操作结构体
 * LCD地址结构体 */
typedef struct
{
    volatile uint16_t LCD_REG; //对应LCD_RS = 0(LCD寄存器)
    volatile uint16_t LCD_RAM; //LCD->LCD_RAM,对应LCD_RS = 1(LCD数据)
} LCD_TypeDef;
