...
/**
 * 数初始化 FSMC 控 制器，同时我们使用 HAL_SRAM_MspInit 回调函数来初始化相应的 IO 口
...
void lcd_init(void)
{
    ...
    //用于初始化 SRAM，注意这个函数不限制一定是 SRAM，只要时序类似，均可使用。前面 说过，这里我们把 LCD 当作 SRAM 使用，因为他们时序类似。
    ...
    /* 尝试9341 ID的读取 */
    //0XD3用于读取ID4指令，读取LCD控制器的ID
    lcd_wr_regno(0XD3);
    //0XD3 指令后面跟了 4 个参数，最后 2 个参数，读出来是 0X93 和 0X41， 刚好是我们控制器 ILI9341 的数字部分，从而，通过该指令，即可判别所用的 LCD 驱动器是什 么型号
