...
{
    //fixme 目前看起来，要与引脚服用，需要和此使能初始化函数放置一块
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    ...
    {
        //以0为索引，所以一共8位，下面位都减1
        //0x80 = 10000000，其中如果g_timxchy_cap_sta第7位也是1（捕获），那么&运算后就是1，反之为0
        if ((g_timxchy_cap_sta & 0X80) == 0)                /* 还未成功捕获 */
        {
            ...
            {
                //第7设置位1
                g_timxchy_cap_sta |= 0X80;                  /* 标记成功捕获到一次高电平脉宽 */ //fixeme
            ...
            else /* 还未开始,第一次捕获上升沿 。第二次就是下降沿，如上的if*/
            {
                ...
                TIM_RESET_CAPTUREPOLARITY(&g_timx_cap_chy_handle, GTIM_TIMX_CAP_CHY);   /* 一定要先清除原来的设置！！ */
                /*因为最初始化时，设置的ICPolarity为TIM_ICPOLARITY_RISING，即上沿捕获，这里拿到了上沿后，需要设置为下沿捕获，然后下沿捕获到后，就继续上沿，如此往复 */
        ...
        if ((g_timxchy_cap_sta & 0X80) == 0)            /* 还未成功捕获 */
        {
            if (g_timxchy_cap_sta & 0X40)               /* 已经捕获到高电平了 */
            {
                ...
                if ((g_timxchy_cap_sta & 0X3F) == 0X3F) /* 高电平太长了 */
                {
                    TIM_RESET_CAPTUREPOLARITY(&g_timx_cap_chy_handle, GTIM_TIMX_CAP_CHY);                     /* 一定要先清除原来的设置 */
                    TIM_SET_CAPTUREPOLARITY(&g_timx_cap_chy_handle, GTIM_TIMX_CAP_CHY, TIM_ICPOLARITY_RISING);/* 配置TIM5通道1上升沿捕获 */
                    g_timxchy_cap_sta |= 0X80;          /* 标记成功捕获了一次 ->  此为无奈之举 */
...
/**
 * @brief       通用定时器TIMX 通道Y 获取当前计数值  ->因为没按下，所以此逻辑为溢出的
 * @param       无
 * @retval      当前计数值
 */
...
{
    uint32_t count = 0;
    //因为定时器每溢出一次的计数个数是 65536，所以这里用 g_timxchy_cnt_ofcnt 乘以 65536，就可以得到溢出计数的个数。前提是ARR寄存器设置为65535
