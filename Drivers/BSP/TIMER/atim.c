...
//{
//    //g_npwm_remain 为5，每次产生一次重载就会产生一次中断，即跑到这个函数。
//    //其中g_timx_npwm_chy_handle.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_ENABLE
//
//    //自动重载寄存器是否具有缓冲作用还受到ARPE位的控制，当该位置0时，ARR寄存器不进行缓冲，我们写入新的
//    // ARR值时，该值会马上被写入ARR影子寄存器中，从而直接生效；当该位置1时，ARR寄存器进行缓冲，我们写入新
//    // 的ARR值时，该值不会马上被写入ARR影子寄存器中，而是要等到更新事件发生才会被写入ARR影子寄存器，这时
//    // 才生效。预分频器寄存器则没有这样相关的控制位，这就是它们不同点。
//
//    //加了volatile才能debug看到变量的值
//    uint16_t volatile npwm = 0;
//
...
//        }
//
//        //上面的g_npwm_remain真的是误导，以为会累加或者递减npwm，结果只是单纯设置main里面的函数用的。。。
//        //所以回到下面的逻辑，当
//        if (npwm) /* 有脉冲要发送 */
//        {
...
//        else
//        {
//            //我自己加了个LED0_TOGGLE()，把main函数的去掉，如果RCR重复计数器为空时，就会重制定时器，我觉得这样更直观
//            LED0_TOGGLE();
...
void atim_timx_comp_pwm_init(uint16_t arr, uint16_t psc)
{
    ...
    //http://www.360doc.com/content/14/1211/10/6828497_432083097.shtml
    //输出比较模式下翻转功能作用是：当计数器的值等于捕获/比较寄存器影子寄存器的值时，
    // OC1REF 发生翻转，进而控制通道输出（OCx）翻转。通过翻转功能实现输出 PWM
    // 具体原 理如下：PWM 频率由自动重载寄存器（TIMx_ARR）的值决定，在这个过程中，
    // 只要自动重载 寄存器的值不变，那么 PWM 占空比就固定为 50%。我们可以通过捕获/比较寄存器（TIMx_CCRx） 的值改变 PWM 的相位。
...
void atim_timx_cplm_pwm_init(uint16_t arr, uint16_t psc)
{
    ...
    //HAL_TIM_PWM_Start 函数和 HAL_TIMEx_PWMN_Start 函数启动通 道输出和互补通道输出。
    //TIM1->PE9  OCy正常
    //TIM1->PE8  OCyN互补
    //TIM1->PE15 BREAK刹车输入
...
void atim_timx_cplm_pwm_set(uint16_t ccr, uint8_t dtg)
{
    ...
    __HAL_TIM_MOE_ENABLE(&g_timx_cplm_pwm_handle);  /* MOE=1,使能主输出  位 MOE 是使能主输出，想要高级定时器的通道正常输出，则必须设置 MOE 位为 1。 */
...
static void atim_timx_pwmin_chy_process(void)
{
    ...
    //表示第一次采集到捕获中断。这时候相当于第一次 捕获到上升沿，我们只是把 sflag 标志位置 1，然后清除所有中断标志位，等待下次的捕获中断 发生。如果再次发生捕获中断
